...
public class PreAuthServiceImpl implements PreAuthService {
    ...
    public PreAuthService getPendingBalance(String gcardNbr, Handler<AsyncResult<JsonObject>> resultHandler) {
        ...
        sqlClient.queryWithParams("SELECT current_bal FROM gcard where gcard_nbr = ?", record, select -> {
            if (select.failed()) {
                LOGGER.error("Failed to get gift card balance", select.cause());
                ...
                JsonObject response = new JsonObject();
                response.put("balance", select.result().getResults().get(0).getDouble(0));
